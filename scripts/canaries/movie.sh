#!/usr/bin/env bash
# Movie canary: verify Jellyseerr → Radarr → import propagation by
...
# Jellyseerr → Radarr connection live + at least one movie in Radarr
RES=$(sshm '
...
# 1. Jellyseerr → Radarr server registered + reachable
...
[ "${JS_RADARR:-0}" -ge 1 ] || { echo "FAIL: Jellyseerr has 0 Radarr servers" >&2; exit 1; }
