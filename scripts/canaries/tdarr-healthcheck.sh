...
#
# Two independent predicates, because a ratio alone is a lagging indicator:
...
if completed < min_sample:
    out("PASS-WARN: tdarr-hc-only-%d-completed-checks-need-%d-to-judge-queued=%d-%s"
        % (completed, min_sample, queued, libstr))

ratio = (errored * 100.0) / completed
tail = ("errors=%d/%d-%.1f%%-queued=%d-warn=%.0f%%-fail=%.0f%%-%s"
        % (errored, completed, ratio, queued, warn_pct, fail_pct, libstr))
