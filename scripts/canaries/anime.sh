#!/usr/bin/env bash
# Anime canary: verify Seerr → Sonarr2 routing for the anime branch.
set -euo pipefail
...
# Seerr Sonarr server count (need >=2: regular + anime)
...
[ "${JS_SONARR:-0}" -ge 2 ] || { echo "FAIL: Seerr has <2 Sonarr servers (need anime + default)" >&2; exit 1; }
