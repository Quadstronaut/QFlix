...
def plan_for_share(
    *,
    ...
    # The typo backstop must be LOUDEST at the moment of harm, not silent.
    # `never_seen` was alerted only while PENDING, so the single signal that
    # separates "they never subscribed" from "we wrote their address down
    # wrong" went quiet on exactly the run that took their libraries away.
    expired_alert = None
    if answer.never_seen:
        expired_alert = (
            "REDUCED %s -- and the entitlement service has NEVER SEEN %s. If "
            "that address is a typo in billing.holder, this reduction is wrong "
            "and is reversed by fixing the roster (no re-invite needed)."
            % (mask(email), mask(holder or "?")))
