...
# The four functions below are thin wrappers around lib.page_dedup — the
# ONE cross-run page-dedup mechanism in the repo (council round 2, 2026-09-17).
# They read the module globals ABOVE (_ESCALATION_PAGE_LEDGER,
# _ESCALATION_PAGE_COOLDOWN_S) on EVERY call, not at import time: the existing
# test suite (test_recovery_escalation_page_cooldown.py) monkeypatches those
# attributes at runtime via `monkeypatch.setattr(recovery_mod, "_ESCALATION_..."
# ...)`, and a wrapper that captured either at import/module-load time would
# silently keep testing the wrong ledger. Names, signatures and behaviour all
# stay byte-identical to what they replace — only the two deliberate deltas
# named in the module they now delegate to: the ledger file is 0600 on POSIX,
# and expired keys are pruned on write. Neither changes any page decision.
