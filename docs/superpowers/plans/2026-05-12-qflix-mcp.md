...
| Path | Responsibility |
|---|---|
| `scripts/mcp/lib/qbit_client.py` | qBit WebUI auth + list torrents (stateless client). `lib/` is a **namespace package** — no `__init__.py`; merges with `scripts/maint/lib/` and `scripts/local/qflix-mcp/lib/` so all three contribute modules to a shared `lib` import root. Collisions avoided by suffixed names. |
| `scripts/mcp/lib/maint_state.py` | Reads `~/.opt/maint/state.json` for Kuma-red gating |
...
| `scripts/local/qflix-mcp/lib/cache.py` | Read/walk `B:\QFlix\data\snapshots`, atomic writes. (no `__init__.py` — namespace package, see above) |
...
| `scripts/local/qflix-mcp/lib/kuma_push.py` | Push to Kuma push monitor |
...
| `tests/unit/test_mcp_maint_state.py` | Tests for Kuma-red state reader |
...
| `tests/unit/test_qflix_mcp_kuma_push.py` | Tests for kuma wrapper |
...
- Note: do NOT create `scripts/mcp/lib/__init__.py` — `lib/` is a namespace package so it merges with `scripts/maint/lib/`. Adding an `__init__.py` would shadow the maint lib and break existing tests.
...
- [ ] **Step 3: Implement `scripts/mcp/lib/qbit_client.py`**
...
```bash
git add scripts/mcp/lib/qbit_client.py tests/unit/test_mcp_qbit_client.py
git commit -m "feat(mcp): qBit WebUI client (login + list_torrents)"
```

---

### Task 2: `scripts/mcp/lib/maint_state.py`

**Files:**
- Create: `scripts/mcp/lib/maint_state.py`
- Create: `tests/unit/test_mcp_maint_state.py`

- [ ] **Step 1: Write the failing test**

`tests/unit/test_mcp_maint_state.py`:
...
- [ ] **Step 3: Implement `scripts/mcp/lib/maint_state.py`**
...
- Note: do NOT create `scripts/local/qflix-mcp/lib/__init__.py` — namespace package (see Task 1 note).
...
- [ ] **Step 3: Implement `scripts/local/qflix-mcp/lib/cache.py`**
...
```bash
git add scripts/local/qflix-mcp/lib/cache.py tests/unit/test_qflix_mcp_cache.py
...
### Task 13: `scripts/local/qflix-mcp/lib/kuma_push.py`

**Files:**
- Create: `scripts/local/qflix-mcp/lib/kuma_push.py`
- Create: `tests/unit/test_qflix_mcp_kuma_push.py`

- [ ] **Step 1: Write the failing test**

`tests/unit/test_qflix_mcp_kuma_push.py`:
...
- [ ] **Step 3: Implement `scripts/local/qflix-mcp/lib/kuma_push.py`**
...
- **Path conventions.** Repo root is `G:\Documents\GIT\Ultra.cc\QFlix` (Windows) or `~/QFlix` (seedbox checkout if any). Workstation absolute paths use `B:\QFlix\data\`; seedbox absolute paths use `~/scripts/mcp/`.
