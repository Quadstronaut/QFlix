...
| R4 | S2 untracked file bodies — `scripts/local-llm/qflix-rea.ps1`, `scripts/manitoba-tunnel.ps1`. | Only their extracted **policy** is guarded (`manifest/rea-noise-classes.yaml`, C-07). A body edit that does not touch policy is invisible to CI, and that part cannot be closed while the files stay out of a public repo. **What changed 2026-08-03: it is no longer also _unrecoverable_.** `scripts/local-llm/backup-untracked.ps1` reads the S2 member list, copies each file hourly into the same `B:\BAKS\Documents` mirror `Backup-Documents` uses, keeps a content-addressed version history *outside* the `/MIR` purge scope, SHA256-verifies all three, and exits non-zero + pages Discord on any divergence. Measured the day it landed: the mirror was **22 hours and two edits behind** `qflix-rea.ps1` while every surface reported success. **What changed 2026-08-19: the REA policy surface count went 4 → 2, so policy drift is no longer a thing a human can cause.** It used to live on four surfaces — yaml `classes`, yaml `prompt_segments`, the `$Script:NoiseFindingRules` literal in the ps1, and the ps1's "NEVER report" prompt sentence — and the untracked pair fell behind the tracked pair three times in three weeks (2026-07-29, 2026-08-06, 2026-08-19; the last left the yaml at 27 classes and the ps1 at 25, C-07 red, `report_digest` host-dependent, and both new classes **inert** while REA kept paging on them). The two ps1 surfaces are now **derived, never authored**: `scripts/local-llm/rea-noise-classes.ps1` (tracked) `Get-ReaNoiseRules` hands REA the live table straight from the yaml, and `Sync-ReaNoiseMirror` — called from `Invoke-Main`, non-fatal — regenerates the ps1's inert text mirror of that table and appends any never-report clause the prompt is missing. A class added to the yaml **alone** is enforcing on the next run and leaves C-07 clean, with no ps1 edit; proven by `tests/local-llm/test-rea-noise-classes.ps1` ("a yaml-only class reaches both surfaces with no ps1 edit"). What is *still* residual: the sync can only **stub** prompt prose from `marker` + `prompt_clause`, so the human-written explanation of *why* a class is benign — and of the sibling shape that must still page — remains a hand-authored, unverified quality. And the body of the ps1 outside the two generated regions is as invisible to CI as it ever was. | operator | 90 | 2026-08-19 |
...
| R7 | **A broken LINK between two apps that are both individually green.** The REA ownership gate (added 2026-09-02) holds any connection failure whose target port resolves in `secrets/*.port`, on the grounds that the target has its own Kuma monitor and the page duplicates it green *or* red. A link that is broken while both endpoints are up is therefore held and will not page. | Deliberate, not an oversight, and the alternative is worse: before the gate, one root cause produced 45 REA pages in 24h of which 2 were real, and a channel nobody reads protects nothing. Per the compartmentalize law a link that matters gets its **own** canary with its own timer and Kuma check (the `bazarr2 -> sonarr2` SignalR negotiate path is the live example and has none). Partial cover: held findings are written to the audit log by rule id and counted on the daily heartbeat, which no longer titles itself "clean" when its only findings were repeats, so a persistent one is visible in the next day's review rather than silent. Closing this properly means writing the canary, not loosening the gate. | operator | 90 | 2026-09-02 |
