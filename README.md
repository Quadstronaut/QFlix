...
apps/
  ...
  maint/                     # the maintenance daemon
    ...
    systemd/                 # 55 services + 43 timers (as of 2026-07-29 — this
                             # grows with every new janitor/canary; re-count via
                             # `systemctl --user list-unit-files` rather than
                             # trusting this comment once it's gone stale) → ~/.config/systemd/user/
