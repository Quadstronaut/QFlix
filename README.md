...
<p>
  ...
  <a href="#operator-visibility"><img alt="Kuma" src="https://img.shields.io/badge/Kuma-83%2F83_up-d4af37?style=for-the-badge&labelColor=0a1628"></a>
...
| End-to-end canaries (`manifest/apps.yaml` `canaries:`) | **40** | movie · anime · mobile-ux · qbit-stall · sab-stall · bazarr-ingest · tdarr-pause-integrity · stream-cap-liveness · cron-liveness · unstick-rate · vlogs-stall · kometa-libraries · stale-log-watchdog · kometa-deploy-drift · prowlarr-indexer-health · prowlarr-app-sync · hardlink-integrity · library-container-sanity · plex-transcoder · plex-playback · plex-unmatched · plex-intro-markers · tautulli-plex-link · quota · newsletter-digest · thread-ceiling · tdarr-scanner · tdarr-healthcheck · tdarr-transcode-stall · ucc-gate-stuck · dash-asset-integrity · timer-liveness · deploy-drift · rea-liveness · arr-plex-parity · prowlarr-proxy-link-fatal · plex-decision-stable-file · seerr-arr-parity |
| Kuma push monitors (manitoba-owned) | **83** | 31 manifest apps + 40 canaries + 1 pusher self-heartbeat + 1 fleet-aggregate + 1 "QFlix Reaper" + 1 "QFlix Audio Disposition" + 1 "qflix-anime-janitor" + 1 "QFlix Torrent Janitor" + 1 "QFlix Unknown Codec Stream" + 1 "QFlix Poster Janitor" + 1 "QFlix Collect" + 1 "QFlix Audit Regime" + 1 "QFlix Audit Live" + 1 "QFlix Entitlement Gate". ("34 canaries" here read stale against the **35** on the row above until 2026-08-24 — the guard only enforces the bolded total, so the breakdown drifted unseen.) "QFlix Poster Janitor" (declared 2026-08-24) is the newest and is `manifest_only` until `bootstrap-kuma-monitors.py` next runs on the box. 0 external: "QFlix Collect" was reclassified manitoba-owned on 2026-07-29 because the collector moved off the workstation onto the box on 2026-07-09, so the drift audit and `bootstrap-kuma-monitors.py` now treat it as mandatory. Its Kuma name still reads "(workstation)" — cosmetic only; renaming needs a coordinated live change. **All 74 are live** as of 2026-08-16: `manitoba-maint kuma audit` reported `manifest 78 / kuma 78 / matched 78, no drift` on 2026-08-08; the books stack was decommissioned 2026-08-16 and its four monitors deleted, taking both sides to 74. The three canary monitors declared 2026-08-03 ("Canary Prowlarr App Sync", "Canary Plex Unmatched", "Canary REA Liveness") were created by `bootstrap-kuma-monitors.py` on a subsequent deploy, as was "Canary Bazarr Ingest" (2026-08-06) — see [docs/audit-regime.md](docs/audit-regime.md). A freshly created monitor is born BOTH tokenless and mute, and both halves are repaired in the same bootstrap run: Bazarr Ingest came up `+ channels [1, 2] (was NONE)` and needed its push token captured by a live `getMonitor` round trip before the install smoke would pass (63/63). "Canary Seerr Arr Parity" (declared 2026-09-13, the Seerr/*arr cross-system reconciliation gap — spec section 4) is the newest and is `manifest_only` until `bootstrap-kuma-monitors.py` next runs on the box - a true finding while it lasts, not drift noise, and it is born BOTH mute and tokenless until that run reconciles both halves. |
| Cron + systemd timers | **61** _(live count on the box, verified 2026-08-07 via `systemctl --user list-timers` — 46 tracked in `scripts/maint/systemd/` plus the 7 installer-generated units now declared in `manifest/jobs.yaml` via `unit:`, plus the 2 canary timers added 2026-08-07, plus `manitoba-maint-poster-janitor.timer` added 2026-08-24, plus `manitoba-maint-canary-arr-plex-parity.timer` added 2026-08-26, plus `manitoba-maint-canary-prowlarr-proxy-link-fatal.timer` added 2026-09-10, plus `manitoba-maint-canary-plex-decision-stable-file.timer` added 2026-09-12, plus `manitoba-maint-canary-seerr-arr-parity.timer` added 2026-09-13. `timer-liveness` asserts every declared one is loaded+active+scheduled: 54/54 at last run, 60 once stream-cap-liveness, poster-janitor, arr-plex-parity, prowlarr-proxy-link-fatal, plex-decision-stable-file and seerr-arr-parity deploy.)_ | window-aware (Mon 11–15 UTC drain) |
...
| 🟠 Retention | **qflix-reaper** | **Per-file 45-day retention** (option D, shipped 2026-09-15): every episode file / movie file is available to members for 45 days from the moment Plex imported it — graded on the Plex **leaf** `addedAt` corroborated by the *arr's `dateAdded` (always the newer; the Plex *container* clock and file mtime are rejected — the container clock deleted Futurama 70 minutes after it was requested on 2026-09-12). Whether a show is airing is not a consideration. Deleting an episode file unmonitors its episode(s) in the same operation so Sonarr never re-grabs what just expired; a series RECORD is removed only when the show has **ended AND holds zero files** and does not carry the `permanent` tag (`scripts/maint/qflix-permanent.py --auto` tags every continuing series; a permanent show keeps its record at zero files so new episodes keep arriving), and the Plex show item is deleted with it (a removed folder is never re-scanned, so refresh+emptyTrash left ghosts). Floor `MIN_FILE_AGE_FLOOR_DAYS=45` is raise-only; a file with no determinable clock is withheld, counted and named. Script-driven (`scripts/maint/qflix-reaper.py`, daily timer, `--max-items 50` shared between file deletes and record removals, audit manifest with `series_removals[]` written before any delete, run lock; the 30%-per-library tripwire is disabled live via the on-box `--max-pct 100` drop-in — operator decision 2026-07-13). Media that can't resolve to a single *arr id is an **orphan** — never deleted, and graced: a fresh orphan reds the run for 24h, then downgrades to a weekly WARN ([2026-07-14 spec](docs/superpowers/specs/2026-07-14-reaper-orphan-grace-design.md)). Seerr reconciliation sweeps every status and clears stuck DELETED seasons so reaped titles are re-requestable; a member's live request on a cleared row is re-created on their behalf (the clear cascades requests — Yellowjackets 2026-09-15), and a row whose requests cannot be read is withheld. Plex-watchlist auto-request is disabled for every member (operator ruling 2026-09-15): requests are made in Seerr, never inferred from a watchlist. Spec: [2026-09-12 per-file retention](docs/superpowers/specs/2026-09-12-reaper-d-per-file-retention-spec.md). Replaced Maintainerr 2026-06-20. |

Surrounding cast (qBittorrent, SABnzbd, FlareSolverr, Tautulli, Tdarr, Listmonk, qflix-newsletter, Buildarr, Recyclarr, Kometa, qflix-dash, Kuma, manitoba-maint, 40 canaries, python-plexapi venv, postgres, unpackerr, upgradinatorr): same single-source-of-truth manifest, same maintenance window. Download clients are now dual-stack: **torrent** (qBittorrent) and **Usenet** (SABnzbd + NZBgeek indexer + Frugal block account). Full breakdown in [`inventory.md`](inventory.md).
...
flowchart LR
  ...
  kuma[(Uptime Kuma<br/>isolated netns · 69 push monitors)]:::kuma
...
```text
manifest/apps.yaml           # 31 apps + 40 canaries — single source of truth
...
apps/
  ...
  canaries/                  # 40 end-to-end pipeline checks (bash)
  ...
  maint/                     # the maintenance daemon
    ...
    systemd/                 # 61 services + 61 timers LIVE in ~/.config/systemd/user/
                             # (2026-09-12; 59 + 45 once dash-asset-integrity
...
tests/                       # 2690 pytest tests (unit/) — pure-Python, no SSH;
                             # plus 477 PowerShell cases (local-llm/) guarding
                             # the REA alerting layer, run by the `pwsh` CI job
```
...
- **QFlix Random Error Audit (REA).** Workstation-side second-opinion audit (`scripts/local-llm/qflix-rea.ps1` — gitignored; Task Scheduler at `\Archangel\QFlix-LLM\QFlix Random Error Audit`, AtLogOn trigger). On every Windows logon, after the SSH tunnel is up, it pulls 14 seedbox log surfaces (*arr logs, systemd journal errors, cron mail spool, maint pipeline, nginx errors, Plex errors, Kuma red-state, SABnzbd, Tdarr, Kometa, config-sync (buildarr/recyclarr/bazarr2), app extras (dash/newsletter/unpackerr), reaper daily log, VictoriaLogs aggregate) in **one SSH call** (widened from 7 to 14 sources 2026-07-25 — see CHANGELOG.md), then hands the consolidated blob to every code-capable Ollama model installed locally (auto-discovered via regex — `qwen3-coder:30b`, `qwen2.5-coder:7b`, `qwen3:8b` today). Models run sequentially, then a finding has to survive **four independent floors** before it reaches Discord — rebuilt 2026-09-02 after a measured 45 pages in 24h of which **2 were real** (operator: *"if I'm getting pinged 40+ times that's just noise, not a useful alert"*). (1) **Noise policy** — 39 enforced classes from `manifest/rea-noise-classes.yaml`, each carrying written evidence and a sibling shape that must still page. (2) **Ownership gate** — REA reads *logs*, which are history, while Kuma and the 40 canaries measure *live state*, so a connection failure whose target port resolves in `secrets/*.port` is held: that target has its own monitor and the page is a duplicate whether the monitor is green (already resolved) or red (already paged). REA's job is the **residue** — faults nothing else can see — and an unmanaged target still pages. (3) **Consensus floor** — findings merge on a collector-anchored *excerpt* key, never on the model-authored `signature` (models re-invent that hourly, so one bazarr line once paged as four), and a finding only one model flagged never pings. (4) **Cross-run page ledger** — a key that already paged stays quiet for 24h, so an unchanged fault cannot page 72 times over its 3-day freshness window; the stamp clears on recovery, making the cooldown per-*outage*. Every floor **fails open**, and every held finding is written to the audit log by rule id and counted on the daily heartbeat — which no longer titles itself "clean" when its only findings were already-paged repeats. Survivors become one Discord message with the operator @ping, or a daily "✓ clean" heartbeat if nothing does. If Ollama itself is unreachable, a separate dead-man Discord alert fires (24h dedupe). Spec: [`docs/superpowers/specs/2026-05-11-qflix-rea-design.md`](docs/superpowers/specs/2026-05-11-qflix-rea-design.md). Install: `scripts/local-llm/qflix-rea.ps1 -Install`. Not wired into Kuma — purely local set of eyes.
...
| Seerr (requests) | `https://seerr-<fqdn>/` (its own vhost since 2026-09-15; `https://<fqdn>/seerr/` 301s there — the sub-path shim broke search/reload on phones) | Plex SSO |
