...
**Not resolved — needs operator decision: BUILDARR.**

- ✗ **`buildarr-sonarr 0.6.4` and `buildarr-radarr 0.2.6` (both latest on PyPI) are incompatible with the running Sonarr v4.0.17 / Radarr v5.** Verified live with real API keys + populated config: sonarr connection test passes, then `KeyError: 'preferred'` during release-profile fetch (buildarr-sonarr expects a field the current API doesn't return). With sonarr removed and radarr-only config: `pydantic.ValidationError: NamingConfigResource.colonReplacementFormat — value is not a valid enumeration member` (radarr v5 added a new enum value to colon-replacement). Upstream plugin maintenance has stalled; no path forward without forking + patching. Manifest entry kept on legacy `systemd_only` probe against `buildarr.timer` (preserves the prior silent-failure behavior — better than ping-storming the operator every Monday until the issue is decided). Original commented-out `buildarr.yml` restored. **Open options: (a) park buildarr (disable timer + `parked: true` in manifest + delete Kuma monitor); (b) fork+patch the plugins; (c) wait for upstream.**
