...
| `B:\Tools\victorialogs\victoria-logs-windows-amd64-prod.exe` + `scripts\local\qflix-vlogs-ship.ps1` + `scripts\local\install-qflix-vlogs.ps1` | windows binary + powershell shipper + installer | server: Windows Task `\Archangel\QFlix-Logging\VictoriaLogs` (AtLogOn, long-running) — shipper: Windows Task `\Archangel\QFlix-Logging\Ship Logs` (every 5 min while logged on) | **QFlix Logging — workstation-local VictoriaLogs.** Persistent searchable log index for every QFlix app log. Single-binary VictoriaLogs v1.50.0 (~15 MB exe, <512 MB RAM) listening on `127.0.0.1:9428`, embedded storage at `B:\QFlix\logs\` with 90 d retention. Shipper SSH-pulls `~/scripts/mcp/logs.py --emit-json --app all --since 6m` (one SSH call covers every routed app; 1 min overlap accepted for resilience) and POSTs JSON lines to `/insert/jsonline` with stream fields `(host, app)`. The MCP server's `qflix_get_logs` opportunistically write-throughs to the same index on every on-demand audit; `qflix_query_logs` queries it via LogsQL with no SSH hop. Verified end-to-end 2026-05-13: 26,347 lines indexed on first ship. | NO | n/a (workstation-local; never reaches the seedbox) | UI `http://127.0.0.1:9428/select/vmui/` · API `http://127.0.0.1:9428/select/logsql/query` | n/a (workstation; doesn't observe maint window) | Scheduled Task restart settings (1 min interval × 5) + shipper fails silent on transient SSH/POST errors so the 5-min scheduler doesn't email failures | n/a (workstation-local, no Kuma monitor — covered transitively by `QFlix Collect (workstation)` dead-man if the workstation goes dark) | Single PR: #4 (feature) + #5 (binary-glob fix). 5 new unit tests; full suite 404 pass / 5 skip. Install: `scripts/local/install-qflix-vlogs.ps1 -Install` (creates both Scheduled Tasks). Uninstall: `-Uninstall` removes the tasks; leaves binary + data root in place. Operator UI shortcut: `Start-Process msedge 'http://127.0.0.1:9428/select/vmui/'`. |
