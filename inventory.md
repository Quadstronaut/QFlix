...
| ~~maintainerr~~ | ucc | **DECOMMISSIONED 2026-06-20** | ~~Library deletion rules (60-day)~~ → replaced by `qflix-reaper` | n/a | Internal | (still installed; UCC auto-restarts the container → `app-maintainerr uninstall` to fully remove) | n/a | removed from manifest + Kuma (3 monitors deleted) | 0 | Plex-ID resolution bug made deletes unfixable (docs/maintainerr-plex-id-resolution-bug.md). Function now served by `scripts/maint/qflix-reaper.py` (armed `--execute`, daily timer, self-pushes "QFlix Reaper" monitor). **2026-06-24 audit: the container still answers (4 rules / 4 collections) — uninstall to fully decommission.** |
