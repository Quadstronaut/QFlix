...
- **34 apps in `manifest/apps.yaml`** (18 UCC · 6 systemd · 9 cron · 1 library) + 14 canaries (movie, anime, mobile-ux, vlogs-stall, qbit-stall, kometa-libraries, stale-log-watchdog, kometa-deploy-drift, prowlarr-indexer-health, hardlink-integrity, plex-transcoder, tautulli-plex-link, quota, newsletter-digest). **Homarr fully decommissioned 2026-07-13** (uninstalled; was 35 apps / 19 UCC — see row A/homarr).
- **55 Kuma monitors** total: **51 manitoba** (34 app monitors + 14 canary monitors + 1 `Manitoba Pusher` self-heartbeat + 1 fleet-aggregate + 1 `QFlix Reaper`) + 4 external (`Quadstronix`, `Quadstronix Node 1`, `Quadstronix Node 2`, `QFlix Collect (workstation)`). `manitoba-maint kuma audit` shows **no drift** as of 2026-07-13 (manifest 51 = matched 51; the `Homarr` monitor #39 was deleted with the app). The 14th canary `Canary Newsletter Digest` (added 2026-07-09) watches the newsletter's cloud digest blurb for staleness; `Canary Kometa Deploy-Drift` + `QFlix Reaper` were bootstrapped earlier. Every app + all canaries report continuously.
- All 52 manitoba monitors wired to both notification channels (`Mission Control - QFlix` Discord + `Manitoba auto-heal webhook`). No silent-failure drift.
...
| scripts/canaries/{movie,anime,mobile-ux,...}.sh | script | invoked by services above | (see services) | NO | n/a | n/a | n/a | n/a | (via Kuma push) | 13 live canary scripts. README at `scripts/canaries/README.md` documents stage labels + exit-code contract for orchestrator (`manitoba-maint canary push <name>`). (`deletion.sh` retired 2026-06-20.) |
