...
- **35 apps in `manifest/apps.yaml`** (19 UCC · 6 systemd · 9 cron · 1 library) + 14 canaries (movie, anime, mobile-ux, vlogs-stall, qbit-stall, kometa-libraries, stale-log-watchdog, kometa-deploy-drift, prowlarr-indexer-health, hardlink-integrity, plex-transcoder, tautulli-plex-link, quota, newsletter-digest). 2026-07-09: **35/35 apps UP, 14/14 canaries green**.
- **56 Kuma monitors** total: **52 manitoba** (35 app monitors + 14 canary monitors + 1 `Manitoba Pusher` self-heartbeat + 1 fleet-aggregate + 1 `QFlix Reaper`) + 4 external (`Quadstronix`, `Quadstronix Node 1`, `Quadstronix Node 2`, `QFlix Collect (workstation)`). `manitoba-maint kuma audit` shows **no drift** as of 2026-07-09 (manifest 52 = matched 52). The 14th canary `Canary Newsletter Digest` (added 2026-07-09) watches the newsletter's cloud digest blurb for staleness; `Canary Kometa Deploy-Drift` + `QFlix Reaper` were bootstrapped earlier. Every app + all canaries report continuously.
...
| homarr | ucc | yes | Legacy board — superseded as public root by qflix-dash 2026-07-08; still installed | NO | Internal | `/board/private` htpasswd (root now serves qflix-dash) | yes | yes (pusher) | 2 |  |
